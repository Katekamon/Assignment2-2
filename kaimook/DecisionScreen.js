...
import CustomButton from "../kaimook/CustomButton";
                                    ...
                                    function (inError, inRestaurants) {
                                        ...
                                        for (const restaurant of inRestaurants) {
                                            ...
                                            if (this.state.rating !== "") {
                                                if (restaurant.rating < this.state.rating) { passTests = false; }
                                            }
                                            if (this.state.delivery !== "") {
                                                if (restaurant.delivery !== this.state.delivery) { passTests = false; }
                                            }
                                            if (this.state.cuisine.length === 0 && this.state.price === "" && this.state.rating === "" && this.state.delivery === "") {
                                                passTests = true;
                                            }
...
class ChoiceScreen extends React.Component {
    constructor(inProps) {
        super(inProps);
        this.state = {
            participantsList: participants, participantsListRefresh: false,
            selectedVisible: false, vetoVisible: false, vetoDisabled: false, vetoText: "Veto"
        };
    }
    render() {
        return (
                                    ...
                                    if (inValue.vetoed === "no") {
                                        return <TouchableOpacity key={inValue.key}
                                            ...
                                            onPress={() => {
                                                ...
                                                this.setState({
                                                    selectedVisible: false, vetoVisible: false,
                                                    vetoText: buttonLabel, vetoDisabled: !vetoStillAvailable,
        ...
        return (
            <View style={styles.postChoiceScreenContainer}>
                ...
                <View style={styles.postChoiceDetailsContainer}>
                    ...
                    </View>        <View style={styles.postChoiceDetailsRowContainer}>
...
const styles = StyleSheet.create({
    ...
    listScreenContainer: {
        ...
        alignItems: "center",
        justifyContent: "center",
        ...Platform.select({ ios: { paddingTop: Constants.statusBarHeight }, android: {} })
    },
    whosGoingHeadline: {
        fontSize: 30,
        marginTop: 20,
    ...
    selectedDetails: { paddingTop: 80, paddingBottom: 80, alignItems: "center" },
    selectedDetailsLine: { fontSize: 18 },
    vetoContainer: { flex: 1, justifyContent: "center" },
    vetoContainerInner: { justifyContent: "center", alignItems: "center", alignContent: "center" },
